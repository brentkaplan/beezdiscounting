...
* `plot()` for `fit_dd_linear()` fits, with the indifference-point tiers'
  ...
  `"parameters"` (subject `ln k` with t-intervals by condition and the
  condition means overlaid, or a caterpillar without a factor; `k_scale =
  "log10"` shows `k` on a log10 axis as the other tiers do), and `"resid"`
  (transformed-scale residual against `ln(delay)`, the model's regressor).
  `predict(type = "parameters")` returns the per-subject table in
