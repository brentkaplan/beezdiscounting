...
4.  Record both a p-value verdict and a confidence-interval verdict
    ...
    study-relevant N. This df is an empirical calibration device, not a
    df derived from the mixed model (the TMB fit has no exact t sampling
    theory); `df = Inf` recovers the z-test. The two verdicts use the
    same standard error and reference distribution, so they always
    agree; both report formats are returned.
...
- **Type I error calibration** (the primary check): with `delta_k = 0`,
  ...
  The t(n - 2) reference distribution these checks calibrate was adopted
