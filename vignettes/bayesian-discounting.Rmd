...
there are about 30 subjects or fewer. For Rachlin the `logk` anchor is
also delay-unit dependent in a second way: changing the delay unit by a
factor c rescales k by c^s, not c, so the same prior centre describes a
different curve prior whenever s is far from 1. Report a prior-sensitivity
