...
takes minutes), so the outputs shown come from real fits; copy the chunks
into your session to reproduce them.
...
A forest plot makes these contrasts easier to scan than the table. Each row is
a `k` ratio with its confidence interval; the dashed line marks a ratio of 1
(no difference), and contrasts whose interval excludes 1 are highlighted.
Across the condition-by-site grid, the condition contrasts (a threefold effect
by construction) are flagged, whereas the site contrasts (no true effect) are
not.
...
<div class="figure">
<img src="dd-group-comparisons-forest-1.png" alt="Forest plot of pairwise k ratios with confidence intervals across the condition-by-site grid; condition contrasts clear the no-difference line while site contrasts do not."  />
