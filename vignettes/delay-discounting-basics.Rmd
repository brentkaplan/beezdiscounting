...
et al. (2016). On the raw delay axis the long delays span most of the width, so
the few long-delay points dominate `auc_regular` and the short delays, where
most of the curve's change happens, barely count; the log and ordinal scales
spread the points more evenly. `auc_log10` uses `log10(delay + 1)`, which makes
it depend on the delay unit (days vs weeks), so keep one unit when comparing it
across studies. Like
